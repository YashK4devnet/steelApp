    ...
    <div className="min-h-screen bg-gradient-to-b from-[#EEF3FA] to-[#FFFFFF] relative z-0 pb-40">
      ...
      <main className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 pt-2 flex flex-col gap-5">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          {/* Section: Proposed Rate Input (Unit matching asking rate e.g. / Ton or / Truck) */}
          <div className="bg-white rounded-[24px] p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] border border-slate-900/5 flex flex-col gap-3">
            <div>
              ...
              <p className="text-xs font-medium text-text-secondary mt-0.5">
                Enter your final price quote (Per {rateUnit})
              </p>
            </div>

            <div className="relative mt-1">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-base font-extrabold text-primary">
                ₹
              </span>
              <input
                type="number"
                ...
                step="any"
                required
                placeholder={`Enter rate per ${rateUnit.toLowerCase()}`}
                value={proposedRate}
                onChange={(e) => setProposedRate(e.target.value)}
                className="w-full h-12 pl-9 pr-24 bg-slate-50 border border-slate-200 rounded-[14px] outline-none focus:border-primary focus:bg-white text-base font-bold text-text-primary transition-all"
              />
