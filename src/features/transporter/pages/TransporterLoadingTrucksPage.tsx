  ...
  return (
    <div className="flex flex-col gap-4">
      {[1, 2, 3].map((n) => (
        <div 
          key={n} 
          className="bg-white dark:bg-surface rounded-[24px] p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] dark:shadow-[0_8px_24px_rgba(0,0,0,0.35)] border border-slate-900/5 dark:border-white/10 flex flex-col sm:flex-row gap-5 animate-pulse"
        >
          ...
          <div className="flex flex-col justify-center sm:items-end gap-3 border-t sm:border-t-0 sm:border-l border-slate-900/5 dark:border-white/10 pt-4 sm:pt-0 sm:pl-5">
            <div className="sm:text-right space-y-1">
              <div className="h-3 bg-slate-100 dark:bg-slate-800/60 rounded w-20 sm:ml-auto" />
              <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-36 sm:ml-auto" />
            </div>
            <div className="h-10 bg-slate-200 dark:bg-slate-800 rounded-[12px] w-full sm:w-36" />
          </div>
        </div>
      ))}
    </div>
        ...
        <div className="max-w-[1200px] mx-auto flex items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            <button 
              ...
              className="w-10 h-10 bg-white dark:bg-surface rounded-full flex items-center justify-center shadow-[0_2px_8px_rgba(15,23,42,0.04)] border border-slate-900/5 dark:border-white/10 text-text-primary hover:bg-gray-50 dark:hover:bg-slate-800 active:scale-95 transition-all cursor-pointer"
            >
              <ArrowLeftIcon />
            </button>
        ...
        <main className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 pt-4 flex flex-col gap-4">
          {loading ? (
            ...
            filteredTrucks.map((truck) => (
              <div 
                key={truck.id}
                className="animate-filter-scale bg-white dark:bg-surface rounded-[24px] p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] dark:shadow-[0_8px_24px_rgba(0,0,0,0.35)] border border-slate-900/5 dark:border-white/10 flex flex-col sm:flex-row gap-5"
              >
                {/* Icon & Primary Info */}
                <div className="flex flex-row items-center gap-4 flex-1">
                    ...
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className={`text-[11px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full ${
                        truck.is_bilty_submitted 
                          ? 'bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-950/60 dark:text-emerald-400 dark:border-emerald-800/40' 
                          : 'bg-blue-50 text-blue-600 dark:bg-blue-950/60 dark:text-blue-400 dark:border dark:border-blue-800/40'
                      }`}>
                ...
                <div className="flex flex-col justify-center sm:items-end gap-3 border-t sm:border-t-0 sm:border-l border-slate-900/5 dark:border-white/10 pt-4 sm:pt-0 sm:pl-5">
                  ...
                  <button 
                    ...
                    className={`w-full sm:w-auto px-6 py-2.5 rounded-[12px] font-semibold text-[14px] transition-all ${
                      truck.is_bilty_submitted 
                        ? 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border border-slate-200 dark:border-white/5 cursor-not-allowed shadow-none' 
