 ...
 */
export async function getOutgoingTrucks(): Promise<OutgoingTruck[]> {
...
export async function submitVendorBill(payload: SubmitVendorBillPayload): Promise<{ status: string; loading_id?: number; truck_line_id?: number }> {
  return apiRequest<{ status: string; loading_id?: number; truck_line_id?: number }>(
    'POST',
    '/booking/trucks/submit_vendor_bill',
    payload
  );
}
...
export async function reportTruckArrival(payload: {
  ...
  return apiRequest<{ status: string; truck_line_id: number }>(
    'POST',
    '/booking/trucks/report',
    payload
  );
}
...
export async function reportOutgoingTruckArrival(payload: {
  ...
  return apiRequest<{ status: string; truck_id: number }>(
    'POST',
    '/booking/trucks/outgoing/report',
    payload
  );
}
